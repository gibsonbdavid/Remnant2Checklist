function findEntries(htmlContent) {
  const galleryRegex =
    /<a[^>]*class\s*=\s*(['"])row gallery\1[^>]*>[\s\S]*?<\/a>/i;
  ...
  const aTagRegex =
    /<a[^>]*class\s*=\s*(['"])wiki_link wiki_tooltip\1[^>]*>[\s\S]*?<\/a>/gi;
  ...
  while ((match = aTagRegex.exec(htmlContentAfterGallery)) !== null) {
    ...
    if (titleAttrMatch) {
      ...
      if (index !== -1) {
        const entryName = titleAttrValue
          .slice(index + "Remnant 2".length)
          .trim();
        ...
        if (foundEntries.has(entry)) {
          if (entry != "Healing Shot/Healing+Shot") {
            console.log("break");
            break;
          } else {
            continue;
          } // Exit the loop once a repeat entry is found
        } else {
