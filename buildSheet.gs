function buildSheet(url, type) {
  ...
  //console.log(entriesCombined.length)
...
function buildRow(names, locationUpgradeData, type, gamemode) {
  ...
  for (var i = 0; i < names.length; i++) {
    ...
    if (locationUpgradeData[i] != undefined && locationUpgradeData[i][0] != null) {
      console.log(names[i])
      ...
      if (localLocation == undefined) {
        localLocation = ""
      } else if (localLocation == worldLocation) { localLocation = "" }
      if (worldLocation == undefined) { worldLocation = "" }
      
      
      data.push(['=IF(COUNTIF(B2:C2, "False") = 0, "✔", "✘")', "", locationUpgradeData[i][3], type, names[i], gamemode, worldLocation + " " + localLocation, cleanedDetails]);
    }else{

      data.push(['=IF(COUNTIF(B2:C2, "False") = 0, "✔", "✘")', "", "", type, names[i], gamemode, worldLocation + " " + "", ""]);
    }
  }

  var lastRow = SpreadsheetApp.getActiveSheet().getLastRow();
  var checkboxRange = SpreadsheetApp.getActiveSheet().getRange(lastRow + 1, 2, data.length, 1);
