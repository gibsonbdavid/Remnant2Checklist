...
function extractTextFromLiElements(htmlContent) {
  ...
  if (h3Match) {
    ...
    if (ulMatch) {
      ...
      const liTextArray = [...liMatches].map(match => match[1].replace(/<[^>]+>/g, '').trim());
...
function extractTextRAWM(htmlContent){
  ...
  //const h3Regex = /<h3\s+class="bonfire"[^>]*>.*How to.*<\/h3>[\s\S]*?<ul[^>]*>[\s\S]*?<\/ul>/g;
  // Second one for Rings and Amulets
  //const h3Regex = /<h3\s+class="bonfire"[^>]*>.*Location.*<\/h3>[\s\S]*?<ul[^>]*>[\s\S]*?<\/ul>/g;
  const h3Regex = /<h3\s+class="bonfire"[^>]*>.*Where.*<\/h3>[\s\S]*?<ul[^>]*>[\s\S]*?<\/ul>/g;
...
function getLocation(extractedText){
  ...
  const locationLosomn = [/Losomn/,/Beatific Palace/, /Postulant's Parlor/, /Brocwithe Quarter/, /Butcher's Quarter/, /Cotton's Kiln/, /Council Chamber/, /Council Tribunal/, /Ironborough/, /Malefic Gallery/, /Malefic Palace/, /Nimue's Retreat/, /Postulant's Parlor/]
  const locationYaesha = [/Yaesha/,/Kaeula's Rest/,/The Far Woods/,/The Widows Court/,/The Great Bole/,/Forgotten Field/,/The Nameless Nest/,/The Lament/,/The Forbidden Grove/,/Cathedral of Omens/,/Ravager's Lair/]
  const locationNerud = [/N'Erud/,/N'erud/,/Seeker's Rest/,/Phantom Wasteland/,/Vault of the Formless/,/The Hatchery/,/Tower of the Unseen/,/Timeless Horizon/,/Void Vessel Facility/,/Astropath's Respite/,/Alepsis-Taura/,/The Eon Vault/,/Spectrum Nexus/,/The Dark Conduit/]
